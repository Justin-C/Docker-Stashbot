...
async function findItems(itemName) {
    ...
    client.connect();
    try {
        queryResp = await client.query(`SELECT * FROM items WHERE item_name LIKE '%${itemName}%'`);
    } catch (error) {
        console.log('jrc find item error', error);
        client.end();
        return new Error({speechResponse: [STASHBOT_RESPONSE.STASHBOT_RESPONSE.FIND_ERROR()]})
    }
    client.end();
    ...
    if (foundRows.length > 0) {
        for (const entry of foundRows) {
            ...
            responseArray.push({ itemName: entry.item_name, itemLocation: entry.location});
        }
    } else {
