 ...
 * TODO
*/
async function removeFromItems(itemName) {
    let queryResp;
    ...
    if (removedCount === 0 ){
        throw new Error(STASHBOT_RESPONSE.STASHBOT_RESPONSE.REMOVE_NOT_FOUND(itemName));
    } else if (removedCount === 1) {
        const location = queryResp.rows[0].location;
        return STASHBOT_RESPONSE.STASHBOT_RESPONSE.REMOVE_SUCCESS(itemName, location)
    } else {
        throw new Error(STASHBOT_RESPONSE.STASHBOT_RESPONSE.REMOVE_ERROR(itemName));
    }
}

module.exports.removeFromItems = removeFromItems;